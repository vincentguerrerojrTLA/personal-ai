...
class MainActivity:AppCompatActivity(){
 ...
 override fun onCreate(s:Bundle?){super.onCreate(s);setContentView(R.layout.activity_main)
  ...
  val sendButton=findViewById<TextView>(R.id.sendButton);sendButton.contentDescription="Send message";sendButton.isEnabled=false;sendButton.alpha=.45f;input.addTextChangedListener(object:android.text.TextWatcher{override fun beforeTextChanged(s:CharSequence?,st:Int,c:Int,a:Int){};override fun onTextChanged(s:CharSequence?,st:Int,b:Int,c:Int){val ready=!s.isNullOrBlank();sendButton.isEnabled=ready;sendButton.alpha=if(ready)1f else .45f};override fun afterTextChanged(s:android.text.Editable?){}});sendButton.setOnClickListener{send()};input.setOnEditorActionListener{_,id,_->if(id==EditorInfo.IME_ACTION_SEND){send();true}else false}
  val toolsButton=findViewById<TextView>(R.id.toolsButton);toolsButton.contentDescription="Open Kylow tools";toolsButton.setOnClickListener{val opening=tools.visibility!=View.VISIBLE;tools.visibility=if(opening)View.VISIBLE else View.GONE;toolsButton.contentDescription=if(opening)"Close Kylow tools" else "Open Kylow tools";toolsButton.announceForAccessibility(if(opening)"Kylow tools opened" else "Kylow tools closed")}
  ...
  findViewById<Button>(R.id.filesButton).setOnClickListener{append("Kylow","Files is a UI entry point. File importing is not connected yet.")}
  findViewById<Button>(R.id.libraryButton).setOnClickListener{append("Kylow","Kylow Library is a planned local-file surface. Connected storage stays separate until you approve an integration.")}
  fun requestProtectedTool(name:String){android.app.AlertDialog.Builder(this).setTitle(name+" requires approval").setMessage("REQUESTED\n\nThis tool is not enabled automatically. Review its permissions and data access before authorizing it. No access is granted by opening this review.").setPositiveButton("Review"){_,_->append("Kylow",name+" approval review opened. State: REQUESTED. No access has been granted.")}.setNegativeButton("Cancel",null).show()}
  findViewById<Button>(R.id.webButton).setOnClickListener{requestProtectedTool("Web Search")}
  findViewById<Button>(R.id.researchButton).setOnClickListener{requestProtectedTool("Deep Research")}
  findViewById<Button>(R.id.connectedToolsButton).setOnClickListener{requestProtectedTool("Connected Tools")}
  findViewById<Button>(R.id.tasksButton).setOnClickListener{append("Kylow","Tasks is present in the interface; functional task integration is still in progress.")}
  ...
  findViewById<Button>(R.id.filesHomeButton).setOnClickListener{append("Kylow","Local Library is prepared in the interface; file importing is not connected yet.")}
  findViewById<Button>(R.id.webHomeButton).setOnClickListener{requestProtectedTool("Web Search")}
